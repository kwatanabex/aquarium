<?php
/**
 * AdmFormsAq_adm_tableフォーム一覧表示へフォワードするクラス
 *
 * @access    public
 * @package   aquarium
 * @author    {author}
 * @version   $Id:$
 */
class Developer_Adm_Admtable_Index extends AppAction
{
    /**
    ...
    function execute(&$data, &$context)
    {
        ...
        $action =& new Developer_Adm_Admtable_Lst();
        $action->preExecute($data, $context);
        $action->execute($data, $context);
        $action->postExecute($data, $context);
        $context->setTemplateFile('/Developer/Adm/Admtable/Lst.html');
    }
